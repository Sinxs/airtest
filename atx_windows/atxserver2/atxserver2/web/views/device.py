...
class AndroidDeviceControlHandler(AuthRequestHandler):
    ...
    def render_remotecontrol(self, device: dict):
        _addr_ = socket.gethostbyname(socket.gethostname())
        platform = device['platform']
        udid = device['udid']
        if platform == 'android':
            self.render("remotecontrol_android.html", udid=udid, addr=_addr_)
        elif platform == 'apple':
            self.render("remotecontrol_apple.html", udid=udid, addr=_addr_)
        else:
            self.render(
                "error.html",
...
class AppleDeviceListHandler(AuthRequestHandler):
    def get(self):
        _addr_ = socket.gethostbyname(socket.gethostname())
        self.render("applelist.html", addr=_addr_)
...
class DeviceListHandler(AuthRequestHandler):
    ...
    async def get(self):
        ...
        _addr_ = socket.gethostbyname(socket.gethostname())
        self.render("index.html", addr=_addr_)
