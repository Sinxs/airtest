...
def Main(devices):
    class TCmainectype_chapter_nine(unittest.TestCase):
        ...
        def test_Chapter_Nine(self):
            """
            主线关卡第九章战斗流程测试--测试自动通关副本，检测地图卡点，对于没有自动战斗的关卡进入副本后退出
            """
            try:
                self.assertEqual("Duck", main_ectype.test_Chapter_Nine(devices))
            except:
    ...
    srcSuite = unittest.makeSuite(TCmainectype_chapter_nine)
    return srcSuite
