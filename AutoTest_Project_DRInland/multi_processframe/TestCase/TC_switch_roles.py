...
def Main(start, devices):
    class TC_switch_roles(unittest.TestCase):
        u'''测试用例102的集合'''
        ...
        def test_skill(self):
            ...
            try:
                print("开始测试技能模块")
                self.assertEqual("觉醒", skill.switch_roles(start, devices))
            except simplerpc.RpcTimeoutError:
                for i in range(initial.RpcTimeoutime()):  # rpc超时问题重置次数
                    try:
                        printred("————————————————————————————————————"
        ...
        def tearDownClass(self):
            u'''这里放需要在所有用例后执行的部分'''
            set_config("switch_roles")
            goback(devices)
            pass

    srcSuite = unittest.makeSuite(TC_switch_roles)
    return srcSuite
