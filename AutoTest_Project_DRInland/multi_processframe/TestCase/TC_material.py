...
def Main(start, devices):
    class TC_material(unittest.TestCase):
        ...
        def test_longqi(self):
            ...
            """
            try:
                self.assertEqual("日常攻略", material.material(start, devices))
            except simplerpc.RpcTimeoutError:
                for i in range(initial.RpcTimeoutime()):  # rpc超时问题重置次数
                    try:
                        printred(
                ...
                get_screen_shot(start, time.time(), devices, "物资筹备功能测试模块")
    ...
    srcSuite = unittest.makeSuite(TC_material)
    return srcSuite
