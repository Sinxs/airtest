...
def gamemall(devices):
    ...
    for but in Mallbutlist:
        if poco(but).exists():
            common.printgreen("开始点击商城内 " + poco("GameMall(Clone)").offspring(but).offspring("SelectedTextLabel").get_text() + " 子页签")
            poco(but).click()
                ...
                if freeze_poco("GameMall(Clone)").offspring("TabsFrame").child(item1).exists():
                    if freeze_poco("GameMall(Clone)").offspring("TabsFrame").child(item1).get_position()[1] > 0:
                        if freeze_poco("GameMall(Clone)").offspring("TabsFrame").child(item1).offspring("TextLabel").exists():
                            common.printgreen("点击 " + freeze_poco("GameMall(Clone)").offspring("TabsFrame").child(item1).offspring("TextLabel").get_text() + " 页签")
                            freeze_poco("GameMall(Clone)").offspring("TabsFrame").child(item1).click()
                            if poco("ailin").exists():
                                common.printgreen("---------->判断 " + poco("GameMall(Clone)").offspring("TabsFrame").child(item1).offspring("TextLabel").get_text() + " 很穷，没有商品...")
                            elif poco("GameMall(Clone)").offspring("item0").exists():
                                common.printgreen("---------->判断 " + poco("GameMall(Clone)").offspring("TabsFrame").child(item1).offspring("TextLabel").get_text() + " 内容显示正确")
                        else:
                            common.printred("拿到控件，但是却拿不到text，很有可能是程序多写了无用控件.....")
                    else:
                        pass
                else:
                    common.printred("找不到商城子页签，请检查，很有可能是程序多写了无用控件.....")
            if poco("GameMall(Clone)").offspring("ShopFrame").offspring("item0").exists():
                common.printgreen("----------->" + poco("GameMall(Clone)").offspring(but).offspring("SelectedTextLabel").get_text() + " 内子界面显示正确")
                    ...
                    for item in range((len(freeze_poco("TabShopFrame").child()) - 4)):  # 点击子商店的子页签
                        ...
                        if freeze_poco("MallDlg(Clone)").offspring(item2).offspring("TextLabel").exists():  # 商店子页签
                            ...
                            if poco("MallDlg(Clone)").offspring("Panel").child("item0").exists():  # 商店商品
                                ...
                                common.printgreen(poco("MallDlg(Clone)").offspring("TabShopFrame").child(item2).offspring(
                                    "TextLabel").get_text() + " 页签 没有找到任何售卖道具")
                else:
                    if poco("MallDlg(Clone)").offspring("item0").exists():
                        common.printgreen(poco("ShopName").get_text() + " 页面没有其他子页签，界面检测到售卖道具，显示正确")
                    else:
                        common.printgreen(poco("ShopName").get_text() + " 页面没有其他子页签，没有找到任何售卖道具")
                poco("Close").click()
            else:
