    ...
    if poco("XSys_Item_Equip").exists():
        ...
        if poco("ShowTitle").exists():
            ...
            if freeze_poco("TitleDlg(Clone)").offspring("Title").exists():
                ...
                common.printgreen("检查点 " + freeze_poco("TitleDlg(Clone)").offspring("Current").offspring("FightLabel").get_text() + " 显示正确")
                common.printgreen("检查点 " + freeze_poco("TitleDlg(Clone)").offspring("Current").offspring("item0").child("Label").get_text() + " 显示正确")
                common.printgreen("检查点 " + freeze_poco("TitleDlg(Clone)").offspring("Current").offspring("item1").child("Label").get_text() + " 显示正确")
                ...
                if poco("KeepOn").exists():  # 如果有可以晋升的头衔，就点击晋升，但是只点击一次
                    freeze_poco = poco.freeze()  # TODO：定义dongjiepoco
                    common.printgreen("进入头衔晋升界面")
                    common.printgreen("检查点 " + freeze_poco("TitleShareDlg(Clone)").child("Bg").child("Title").get_text() + " 显示正确")
                ...
                if poco("ItemNewDlg(Clone)").offspring("item301").child("Num").exists():  # 判断背包中勋章的数量，如果数量不足以合成就跳过
                    ...
                    if int(poco("ItemNewDlg(Clone)").offspring("item301").child("Num").get_text()) > 3:
                        ...
                        if freeze_poco("TopFrame").offspring("Name").exists():
                            common.printgreen("检查点 " + freeze_poco("Text0").offspring("T").get_text() + "     " + freeze_poco("Text0").get_text() + "      显示正确")
                            common.printgreen("检查点 " + freeze_poco("Text1").offspring("T").get_text() + "     " + freeze_poco("Text1").get_text() + "      显示正确")
                            common.printgreen("检查点 " + freeze_poco("Text2").offspring("T").get_text() + "     " + freeze_poco("Text2").get_text() + "      显示正确")
                            common.printgreen("检查点 " + freeze_poco("Binding").offspring("T").get_text() + "     " + freeze_poco("Binding").offspring("Yes").get_text() + "        显示正确")
                            common.printgreen("检查点 " + freeze_poco("Description").offspring("Title").get_text() + "     " + freeze_poco("Text").get_text() + "        显示正确")
                            ...
                            if poco("Button5").exists():  # 点击勋章升级按钮
                                ...
                                if poco("GreyModalDlg(Clone)").child("Bg").child("Label").exists():
                                    common.printgreen("检查点 " + poco("GreyModalDlg(Clone)").child("Bg").child("Label") .get_text() + "显示正确")
                        ...
                        common.printgreen("战斗勋章不足，不做合成操作")
                        common.get_screen_shot(start, time.time(), devices, "勋章不足")
                else:
                    common.printgreen("背包没有战斗勋章，不做合成操作")
                ...
                if poco("ItemNewDlg(Clone)").offspring("item302").child("Num").exists():  # 判断背包中勋章的数量，如果数量不足以合成就跳过
                    ...
                    common.printgreen("背包没有勋章，不做合成操作")
                    common.get_screen_shot(start, time.time(), devices, "背包没有勋章")
        else:
