        ...
        if freeze_poco(texture="DailyTaskText").exists() and freeze_poco("BaseInfo").exists() and freeze_poco("item0").exists() and \
            ...
            if poco("RefreshLogBtn").exists():
                ...
                if freeze_poco("LogPanel").child("T").exists():
                    ...
                    common.printgreen("检查点  " + freeze_poco("LogPanel").offspring("Title").child("T")[0].get_text() + "  显示正确")
                    common.printgreen("检查点  " + freeze_poco("LogPanel").offspring("Title").child("T")[1].get_text() + "  显示正确")
                    common.printgreen("检查点  " + freeze_poco("LogPanel").offspring("Title").child("T")[2].get_text() + "  显示正确")
            else:
