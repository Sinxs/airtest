                    ...
                    with poco.freeze() as freezez_poco:
                        for i in range(10):
                            ...
                            if not freezez_poco("SpriteSystemDlg(Clone)").offspring("SpriteLotteryHandler").offspring(item).child("ItemTpl").exists():
                                count += 1
                    common.printgreen(f"抽到了{count}个A级以上的坐骑")
                    if count == 0:
                        poco("OkButton").click()
                    else:
                        poco("OkButton").click()
                        ...
                        poco("OkButton").click()
                        common.printgreen("此次抽精灵操作正常")
        except Exception as e:
            common.printred("购买精灵蛋，抽精灵流程异常")
            common.printred(e)
            common.get_screen_shot(start,time.time(), devices, "购买精灵蛋，抽精灵流程正常异常")
        ...
        with poco.freeze() as freeze_poco:
            if freeze_poco("Help").exists() and \
                    ...
                    if freeze_poco("P1").exists():
                        common.printgreen("进阶", end=",")
                    else:
                        print("没有S级以上的精灵，进阶按钮不显示")
                    common.printgreen("精灵技能，上阵，商店，召唤，分解，打造，精灵属性，精灵装备，喂食，觉醒，升星UI元素显示正常")
            else:
                common.printred("精灵界面UI元素显示异常，详情见截图")
                common.get_screen_shot(start,time.time(), devices, "精灵界面UI元素显示异常")
            try:
                ...
                if poco("SpriteSystemDlg(Clone)").offspring("SpriteElves").offspring("item0").child("Item").exists():
                    ...
                    try:
                        ...
                        poco("Button1").click()
                        common.printgreen("精灵装备打造穿戴正常")
                    except Exception as e:
                        common.printred("精灵装备打造穿戴异常")
                        common.printred(e)
                        common.get_screen_shot(start,time.time(), devices, "精灵准备打造穿戴异常")
                else:
                    common.printred("精灵装备界面异常")
                    common.get_screen_shot(start,time.time(), devices, "精灵装备界面异常")
                try:
                    ...
                    with poco.freeze() as preeze_poco: # 冻结计算不是队长位置一共有几个精灵
                        ...
                        if preeze_poco("SpriteSystemDlg(Clone)").offspring("Avatar0").offspring("Name").get_text() == "":
                            ...
                            count += 1
                        common.printgreen(f"一共有{count}个精灵在阵上")
                    ...
                    common.printgreen("精灵上阵操作正常")
                except Exception as e:
                    common.printred("精灵上阵操作异常")
                    common.printred(e)
                    common.get_screen_shot(start,time.time(), devices, "精灵上阵操作异常")
                try:
                # 分解
                    ...
                    with poco.freeze() as freeze_poco:
                        ...
                        common.printgreen("精灵分解成功")
                        freeze_poco("Illustration").click()
                        poco(texture="l_close_00").click()
                    common.printgreen("精灵分解操作正常")
                except Exception as e:
                    common.printred("精灵分解操作异常")
                    common.printred(e)
                    common.get_screen_shot(start,time.time(), devices, "精灵分解操作异常")
            except  Exception as e:
                common.printred("精灵操作流程异常")
                common.printred(e)
                common.get_screen_shot(start,time.time(), devices, "精灵操作流程异常")
    else:
        common.printred("精灵界面报错，详情见截图")
        common.get_screen_shot(start,time.time(), devices, "精灵界面报错")
    poco("Close").click()
