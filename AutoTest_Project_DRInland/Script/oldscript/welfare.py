...
def move(poco,item,devices):
    for i in range(20):
        ...
        if position[1] < 0.12:
            common.setswipe(1, (43, 130), (43, 400), devices)
            time.sleep(1)
        elif position[1] > 0.95:
            common.setswipe(1, (43, 400), (43, 130), devices)
            time.sleep(1)
        else:
...
def zhencang(start,poco,item,devices): # 珍藏礼包
    ...
    with poco.freeze() as freezepoco:
        if freezepoco("WelfareDlg(Clone)").offspring("Grid").child("item0").child("Btn").exists() and \
            ...
            common.printgreen("每日领奖按钮，购买按钮，礼包奖品数目UI元素显示正常")
        else:
            common.printred("珍藏礼包界面缺少UI元素，详情见截图")
            common.get_screen_shot(start,time.time(), devices, "珍藏礼包界面缺少UI元素")
        try:
            ...
            common.printgreen("珍藏礼包界面按钮点击正常")
        except Exception as e:
            common.printred("珍藏礼包界面按钮点击异常")
            common.printred(e)
            common.get_screen_shot(start,time.time(), devices, "珍藏礼包界面按钮点击异常")
...
def panduola(start,poco,item,devices): # 潘多拉
    ...
    with poco.freeze() as freezepoco:
        if freezepoco("WelfareDlg(Clone)").offspring("Pandora").offspring("Display0").offspring("Point").exists() and \
            ...
            common.printgreen("潘多拉模型框，名称，奖励预览，开箱按钮等UI元素显示正常")
        else:
            common.printred("潘多拉界面缺少UI元素，详情见截图")
            common.get_screen_shot(start,time.time(), devices, "潘多拉界面缺少UI元素")
        try:
            poco("ItemList").click()
            ...
            if poco("access212").exists():
                ...
                time.sleep(3)
                poco("OK").click()
            common.printgreen("潘多拉界面按钮点击正常")
        except Exception as e:
            common.printred("潘多拉界面按钮点击异常")
            common.printred(e)
            common.get_screen_shot(start,time.time(), devices, "潘多拉界面按钮点击异常")
...
def yiyu(start,poco,item,devices): # 异域集市
    ...
    with poco.freeze() as freezepoco:
        if freezepoco("ShopItemList").exists() and \
            ...
            common.printgreen("商品框，查看商品，刷新UI元素显示正常")
        else:
            common.printred("异域集市界面缺少UI元素，详情见截图")
            common.get_screen_shot(start,time.time(), devices, "异域集市界面缺少UI元素")
        try:
            ...
            if poco("Cancel").exists():
                poco("Cancel").click()
            common.printgreen("异域集市界面按钮点击正常")
        except Exception as e:
            common.printred("异域集市界面按钮点击异常")
            common.printred(e)
            common.get_screen_shot(start,time.time(), devices, "异域集市界面按钮点击异常")
...
def vip(start,poco,item,devices): # 王国特权
    ...
    with poco.freeze() as freezepoco:
        if freezepoco("WelfareDlg(Clone)").offspring("item0").child("Detail").exists() and \
            ...
            common.printgreen("购买按钮，详情介绍，会员标识UI元素显示正常")
        else:
            common.printred("王国特权界面缺少UI元素，详情见截图")
            common.get_screen_shot(start,time.time(), devices, "王国特权界面缺少UI元素")
        try:
            ...
            common.printgreen("王国特权界面按钮点击正常")
        except Exception as e:
            common.printred("王国特权界面按钮点击异常")
            common.printred(e)
            common.get_screen_shot(start,time.time(), devices, "王国特权界面按钮点击异常")
...
def wangguo(start,poco,item,devices): # 王国礼包
    ...
    with poco.freeze() as freezepoco:
        if freezepoco(texture="l_hy_02").exists() and \
            ...
            common.printgreen("王国礼包图片，奖励列表，购买按钮UI元素显示正常")
        else:
            common.printred("王国礼包界面缺少UI元素，详情见截图")
            common.get_screen_shot(start,time.time(), devices, "王国礼包界面缺少UI元素")
        try:
            poco("WelfareDlg(Clone)").offspring("Grid").child("item0").child("Btn").click()
            poco("WelfareDlg(Clone)").offspring(item).click()
            poco("WelfareDlg(Clone)").offspring("Grid").child("item1").child("Btn").click()
            ...
            poco("WelfareDlg(Clone)").offspring("Grid").child("item2").child("Btn").click()
            poco("WelfareDlg(Clone)").offspring(item).click()
            common.printgreen("王国礼包界面按钮点击正常")
        except Exception as e:
            common.printred("王国礼包界面按钮点击异常")
            common.printred(e)
            common.get_screen_shot(start,time.time(), devices, "王国礼包界面按钮点击异常")
...
def shouchong(start,poco,item,devices): # 首充礼包
    ...
    with poco.freeze() as freezepoco:
        if poco("Recharge").exists() and \
            ...
            common.printgreen("充值按钮，6个奖励UI元素显示正常")
        else:
            common.printred("首充礼包界面缺少UI元素，详情见截图")
            common.get_screen_shot(start,time.time(), devices, "首充礼包界面缺少UI元素")
        try:
            ...
            common.printgreen("首充礼包界面按钮点击正常")
        except Exception as e:
            common.printred("首充礼包界面按钮点击异常")
            common.printred(e)
            common.get_screen_shot(start,time.time(), devices, "首充礼包界面按钮点击异常")
...
def qiandao(start,poco,item,devices): # 签到奖励
    ...
    with poco.freeze() as freezepoco:
        if poco("Panel").exists() and \
            ...
            common.printgreen("会员特权，签到按钮，奖励列表UI元素显示正常")
        else:
            common.printred("签到奖励界面缺少UI元素，详情见截图")
            common.get_screen_shot(start,time.time(), devices, "签到奖励界面缺少UI元素")
        try:
            if poco("Check").child("Cost").exists():
                ...
                poco("Check").click()
            common.printgreen("签到奖励界面按钮点击正常")
        except Exception as e:
            common.printred("签到奖励界面按钮点击异常")
            common.printred(e)
            common.get_screen_shot(start,time.time(), devices, "签到奖励界面按钮点击异常")
...
def jinbi(start,poco,item,devices): # 金币宝箱
    ...
    with poco.freeze() as freezepoco:
        if freezepoco("WelfareDlg(Clone)").offspring("Help").exists() and \
            ...
            common.printgreen("帮助信息，兑换按钮UI元素显示正常")
        else:
            common.printred("金币宝箱界面缺少UI元素，详情见截图")
            common.get_screen_shot(start,time.time(), devices, "金币宝箱界面缺少UI元素")
        try:
            poco("Btn_ExchangeOne").click()
            poco("Btn_ExchangeOne").click()
            poco("Btn_ExchangeTen").click()
            common.printgreen("兑换按钮点击正常")
        except Exception as e:
            common.printred("兑换按钮点击正常")
            common.printred(e)
            common.get_screen_shot(start,time.time(), devices, "兑换按钮点击异常")

def tili(start,poco,item,devices): # 每日体力
    move(poco, item,devices)
    with poco.freeze() as freezepoco:
        if poco("GetReward").exists() or ((poco("L").child("T2").exists() and poco("R").child("T2").exists())):
            common.printgreen("每日体力UI元素显示正常")
        else:
            common.printred("每日体力界面缺少UI元素，详情见截图")
            common.get_screen_shot(start,time.time(), devices, "每日体力界面缺少UI元素")
        try:
            ...
            common.printgreen("每日体力界面按钮点击正常")
        except Exception as e:
            common.printred("每日体力界面按钮点击异常")
            common.printred(e)
            common.get_screen_shot(start,time.time(), devices, "每日体力界面按钮点击异常")
    ...
    if poco("buttons").exists():
        with poco.freeze() as freezepoco:
            if freezepoco("SelectPerfect").exists() and \
                ...
                common.printgreen("完美找回，普通找回，奖励显示正常")
            else:
                common.printred("奖励找回界面缺少UI元素，详情见截图")
                common.get_screen_shot(start,time.time(), devices, "奖励找回界面缺少UI元素")
            try:
                poco("SelectNormal").click()
                poco("SelectPerfect").click()
                poco("WelfareDlg(Clone)").offspring("item0").child("Go").click()
                poco("BtnOK").click()
                common.printgreen("奖励找回界面按钮点击正常")
            except Exception as e:
                common.printred("奖励找回界面按钮点击异常")
                common.printred(e)
                common.get_screen_shot(start,time.time(), devices, "珍藏礼包界面按钮点击异常")
    else:
