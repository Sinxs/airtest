...
def home(start, devices):
    ...
    for item in homelist:
        if freeze_poco("HomeMainView(Clone)").offspring(item).offspring("TextLabel").exists():
            common.printgreen("界面检查点 " + freeze_poco("HomeMainView(Clone)").offspring(item)
                              .offspring("TextLabel").get_text() + " 存在")
        else:
    ...
    if freeze_poco("HomeMainView(Clone)").offspring("HomeLog").child("Tittle").child("NameLab").exists() and \
        ...
        common.printgreen("检查点 " + freeze_poco("HomeMainView(Clone)").offspring("HomeLog").child("Tittle")
                          .child("NameLab").get_text() + " 显示正确")
        common.printgreen("检查点 " + freeze_poco("HomeMainView(Clone)").offspring("FriendsRank").child("Tittle")
                          .child("NameLab").get_text() + " 显示正确")
    ...
    if freeze_poco("ToggleSprite").exists():
        ...
        B = (poco("HomeMainView(Clone)").offspring("UITable").child("item0").child("Children")
            .child("item0").get_position()[1])
        if B - A < 0.05:
            ...
            if poco("SysB_HomeShop").exists():
                ...
                for item in range(len(poco("Panel").child())):
                    ...
                    if freeze_poco("MallDlg(Clone)").offspring(item1).child("Item").child("Name").exists():
                        common.printgreen("界面检查点 " + freeze_poco("MallDlg(Clone)").offspring(item1).child("Item").
                                          child("Name").get_text() + " 显示正确")
                    else:
