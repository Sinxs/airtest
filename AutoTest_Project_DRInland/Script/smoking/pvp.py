    ...
    if poco("SysEPVP").exists():
        ...
        with poco.freeze() as freezepoco:
            if freezepoco("Btn_MobaFamous").exists() and \
                ...
                printgreen("竞技军衔，竞技名人堂，竞技副本显示正常")
            else:
                printred("竞技界面缺少元素，请详细查看")
            ...
            for i in name:
                ...
                if freezepoco(uiname).offspring("Desc").get_text() == "实时对战，领取称号和时装":
                    printgreen("天梯赛--" + freezepoco(uiname).offspring("Desc").get_text())
                elif freezepoco(uiname).offspring("Desc").get_text() == "巅峰对决，跨服对战":
                    printgreen("角斗场--" + freezepoco(uiname).offspring("Desc").get_text())
                elif freezepoco(uiname).offspring("Desc").get_text() == "4V4争夺据点，赢取龙币":
                    printgreen("英雄战场--" + freezepoco(uiname).offspring("Desc").get_text())
                elif freezepoco(uiname).offspring("Desc").get_text() == "释放操作，与全球玩家比拼":
                    printgreen("全球竞技--" + freezepoco(uiname).offspring("Desc").get_text())
                elif freezepoco(uiname).offspring("Desc").get_text() == "保护队长，获得传说装备":
                    printgreen("保护队长--" + freezepoco(uiname).offspring("Desc").get_text())
                else:
                    printgreen("其他的PVP模式--" + freezepoco(uiname).offspring("Desc").get_text())
                count += 1
        if poco("Btn_MobaRank").exists():
            ...
            with poco.freeze() as freezepoco:
                if freezepoco("Date").exists() and \
                    ...
                    printgreen("规则 赛季时间，军衔奖励，挑战记录，排名，军衔，角色，军功元素显示正常")
                else:
                    printred("规则 赛季时间，军衔奖励，挑战记录，排名，军衔，角色，军功元素缺少，请具体核对")
                    get_screen_shot(start, time.time(), devices, "竞技军衔按钮不存在")
            ...
            with poco.freeze() as freezepoco:
                ...
                if count == 8:
                    printgreen("赛季奖励的条目正常")
                else:
                    printred("赛季奖励的条目不是8条，请前往查看少了哪个")
                    get_screen_shot(start, time.time(), devices, "竞技军衔按钮不存在")
            touch([140, 389])
            sleep(1)
            poco("Close").click()
        ...
        if poco("Btn_MobaFamous").exists():
            ...
            with poco.freeze() as freezepoco:
                if freezepoco("item0").exists() and \
                    ...
                    printgreen("1:1天梯赛，英雄战场，全球竞技，跨服巅峰对决，点赞，规则，排行榜按钮显示正常")
                else:
                    printred("1:1天梯赛，英雄战场，全球竞技，跨服巅峰对决，点赞，规则，排行榜按钮显示异常")
                    get_screen_shot(start, time.time(), devices, "天梯赛报错异常")
            if poco("HallFameDlg(Clone)").offspring("Help").exists():
                ...
                with poco.freeze() as freezepoco:
                    if freezepoco("SelectedTextLabel").exists() and \
                        ...
                        printgreen("1:1天梯赛，评分奖励，排名奖励，荣誉商店，排行榜，对战记录，积分排名，开始匹配，技能教学，技能推荐按钮显示正常")
                    else:
                        printred("1:1天梯赛，评分奖励，排名奖励，荣誉商店，排行榜，对战记录，积分排名，开始匹配，技能教学，技能推荐按钮显示异常")
                        get_screen_shot(start, time.time(), devices, "天梯赛报错")
                    try:
                        ...
                        poco("Match1V1Btn").click()
                        poco("Close").click()
                        printgreen("天梯赛所有按钮点击关闭操作正常")
                    except Exception as e:
                        printred("天梯赛界面元素点击报错")
                        printred(e)
                        get_screen_shot(start, time.time(), devices, "天梯赛界面元素点击报错")
                        poco("Close").click()
                ...
                with poco.freeze() as freezepoco:
                    if freezepoco("Help").exists() and \
                        ...
                        printgreen("三个排名奖励，升段奖励，排行榜，荣誉商店，参与宝箱，胜利宝箱，技能推荐按钮显示正常")
                    else:
                        printred("三个排名奖励，升段奖励，排行榜，荣誉商店，参与宝箱，胜利宝箱，技能推荐按钮显示异常")
                        get_screen_shot(start, time.time(), devices, "角斗场报错")
                    try:
                        poco("Help").click()
                        poco("Btn").click()
                        ...
                        if poco("Pause").exists():
                            ...
                            time.sleep(15)
                            poco("SysEPVP").click()
                            printgreen("角斗场进入退出正常")
                            if poco("SysEPVP").exists():
                                poco("SysEPVP").click()
                        else:
                            printred("角斗场进入失败")
                            get_screen_shot(start, time.time(), devices, "角斗场进入失败")
                            poco("Close").click()
                    except Exception as e:
                        printred("角斗场界面元素点击报错")
                        printred(e)
                        get_screen_shot(start, time.time(), devices, "角斗场界面元素点击报错")
                        poco("Close").click()
                ...
                with poco.freeze() as freezepoco:
                    if freezepoco("Help").exists() and \
                        ...
                        printgreen("规则，玩法攻略，技能预览，排行榜，商店，挑战记录，奖励预览，本周奖励，每日首胜，单人匹配，组队匹配，购买按钮，英雄列表等元素显示正常")
                    else:
                        printred("规则，玩法攻略，技能预览，排行榜，商店，挑战记录，奖励预览，本周奖励，每日首胜，单人匹配，组队匹配，购买按钮，英雄列表等元素显示异常")
                        get_screen_shot(start, time.time(), devices, "英雄战场点击报错")
                    try:
                        poco("Help").click()
                        poco("Btn").click()
                        poco("RankBtn").click()
                        ...
                        poco("Close").click()
                        printgreen("英雄战场所有按钮点击关闭操作正常")
                    except Exception as e:
                        printred("英雄战场界面元素点击报错")
                        printred(e)
                        poco("Close").click()
                        get_screen_shot(start, time.time(), devices, "英雄战场界面元素点击报错")
                ...
                with poco.freeze() as freezepoco:
                    if freezepoco("0").exists() and \
                            ...
                            freezepoco("t").exists():
                        printgreen("大标题，介绍内容，全球榜，排行榜，前往挑战按钮，可能获得的奖励显示正常")
                    else:
                        printred("大标题，介绍内容，全球榜，排行榜，前往挑战按钮，可能获得的奖励等元素显示异常")
                        get_screen_shot(start, time.time(), devices, "全球竞技缺少对应按钮")
                    try:
                        ...
                        poco("Close").click()
                        poco("Close").click()
                        printgreen("全球竞技缺所有按钮点击关闭操作正常")
                    except Exception as e:
                        printred("全球竞技缺界面元素点击报错")
                        printred(e)
                        poco("Close").click()
                        get_screen_shot(start, time.time(), devices, "全球竞技缺界面元素点击报错")
                ...
                with poco.freeze() as freezepoco:
                    if freezepoco("CaptainDlg(Clone)").offspring("WeekReward").child("ListPanel").exists() and \
                        ...
                        printgreen("周奖励，日奖励，单人匹配，组队匹配，荣誉商店，对战记录，规则，场次计算显示正常")
                    else:
                        printred("周奖励，日奖励，单人匹配，组队匹配，荣誉商店，对战记录，规则，场次计算显示正常")
                        get_screen_shot(start, time.time(), devices, "保护队长元素报错")
                    try:
                        poco("Help").click()
                        poco("Btn").click()
                        poco("BtnShop").click()
                        ...
                        poco("Close").click()
                        printgreen("保护队长所有按钮点击关闭操作正常")
                    except Exception as e:
                        printred("保护队长界面元素点击报错")
                        printred(e)
                        poco("Close").click()
                        get_screen_shot(start, time.time(), devices, "保护队长报错")
            else:
                printred("界面缺少元素，麻烦检查")
                get_screen_shot(start, time.time(), devices, "竞技军衔按钮不存在")
        else:
            printred("竞技名人堂按钮不存在")
            get_screen_shot(start, time.time(), devices, "竞技名人堂按钮不存在")
    else:
        printred("竞技场暂未开放，请升级或者查看相关配置")
        get_screen_shot(start, time.time(), devices, "竞技功能未开放")
    return poco("Btn_MobaRank").get_name()  # 返回值


if __name__ == "__main__":
    start = time.localtime()
    athletics(start, "9b57691d")
