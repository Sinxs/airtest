        ...
        for guildbut in guildbutlist:
            if poco(guildbut).exists():
                ...
                if guildbut == "BtnSignIn":
                    BtnSignIn(poco, devices)  # 签到
                if guildbut == "BtnGz":
                    BtnGz(poco, devices)  # 工资
                if guildbut == "BtnSkill":
                    BtnSkill(poco, devices)  # 技能
                if guildbut == "BtnRed":
                    BtnRed(poco, devices)  # 红包
                if guildbut == "Donation":
                    Donation(poco, devices)  # 捐赠
                if guildbut == "BtnMall":
                    BtnMall(poco, devices)  # 商店
                if guildbut == "BtnJoker":
                    BtnJoker(poco, devices)  # 小丑扑克
                if guildbut == "Btnfish":
                    Btnfish(poco, devices)  # 钓鱼
                if guildbut == "BtnBuild":
                    BtnBuild(poco, devices)  # 建造
                if guildbut == "BtnConsider":
                    BtnConsider(poco, devices)  # 研究
                if guildbut == "GuildTreasureIcon":
                    ...
                    if freeze_poco("Title").exists() and \
                        ...
                        printcolor.printred("公会宝藏界面缺少控件，请检查")
                        screenshot.get_screen_shot(time.time(), devices, "公会宝藏界面缺少控件")
                    close(poco)
    else:
        printcolor.printred("公会界面缺少控件，请检查")
        screenshot.get_screen_shot(time.time(), devices, "公会界面缺少控件")
    return poco("BtnEnter").child("T").get_text()

def BtnSignIn(poco, devices): # 签到
    freeze_poco = poco.freeze()  # TODO：定义dongjiepoco
    if freeze_poco(texture="public_Flag").exists() and \
        ...
        printcolor.printgreen("工会签到界面缺少控件元素")
        screenshot.get_screen_shot(time.time(), devices, "工会签到界面缺少控件元素")
        close(poco)

def BtnGz(poco, devices):  # 工资
    freeze_poco = poco.freeze()  # TODO：定义dongjiepoco
    if freeze_poco("T").exists() and \
        ...
        if freeze_poco("2").exists() and \
            ...
            printcolor.printred("提升子界面缺少元素，请检查。。。")
            screenshot.get_screen_shot(time.time(), devices, "活跃提升缺少元素")
            poco("Close").click()
    close(poco)


def BtnSkill(poco, devices): # 技能
    freeze_poco = poco.freeze()  # TODO：定义dongjiepoco
    for skill in range(1,len(freeze_poco("SkillList").child())):
        ...
        if freeze_poco(skill1).exists():
            ...
            printcolor.printred("工会技能缺失，请检查。。。")
            screenshot.get_screen_shot(time.time(), devices, "工会技能缺失")
    if freeze_poco("texbg").exists() and \
            freeze_poco("Name").exists() and \
            freeze_poco("Levelup").exists():
        pass
    else:
        printcolor.printred("工会技能界面元素缺失，请检查。。。")
        screenshot.get_screen_shot(time.time(), devices, "工会技能界面元素缺失")
    freeze_poco("Levelup").click()  # 点击升级按钮，没办法做是否成功的判断
    close(poco)


def BtnRed(poco, devices): # 红包
    freeze_poco = poco.freeze()  # TODO：定义dongjiepoco
    if freeze_poco("Title").exists() and \
        ...
        if freeze_poco(texture="l_tip_00").exists() and \
            ...
            printcolor.printred("红包池界面元素缺失，请检查。。。")
            screenshot.get_screen_shot(time.time(), devices, "红包池界面元素缺失")
            poco(texture="l_close_00").click()
    else:
        printcolor.printred("工会红包界面元素缺失，请检查。。。")
        screenshot.get_screen_shot(time.time(), devices, "工会红包界面元素缺失")


def Donation(poco, devices):  # 捐赠
    freeze_poco = poco.freeze()  # TODO：定义dongjiepoco
    if freeze_poco("T").exists() and freeze_poco("RankBtn").exists():
        ...
        if freeze_poco("Tittle").exists() and \
            ...
            if freeze_poco(text="排名").exists() and \
                ...
                printcolor.printred("捐赠历史排行界面元素缺失，请检查。。。")
                screenshot.get_screen_shot(time.time(), devices, "捐赠历史排行界面元素缺失")
                poco(texture="l_close_00").click()
        else:
            printcolor.printred("捐赠今日排行界面元素缺失，请检查。。。")
            screenshot.get_screen_shot(time.time(), devices, "捐赠今日排行界面元素缺失")
            poco(texture="l_close_00").click()
    ...
    if poco("Tittle").exists():
        ...
        printcolor.printred("捐赠记录没有弹出，请检查。。。")
        screenshot.get_screen_shot(time.time(), devices, "捐赠记录没有弹出")
    poco("WeeklyDonation").click()  # 点击悬赏捐赠
    if poco("Bg2").exists():
        close(poco)
    else:
        printcolor.printred("悬赏捐赠，请检查。。。")
        screenshot.get_screen_shot(time.time(), devices, "悬赏捐赠")
...
def BtnMall(poco, devices):  # 商店
    ...
    if poco("OK").exists():
        ...
        printcolor.printgreen("不能购买商品")
    close(poco)

def BtnJoker(poco, devices):  # 小丑扑克
    freeze_poco = poco.freeze()  # TODO：定义dongjiepoco
    if freeze_poco(text="小丑扑克").exists() and \
        ...
        if poco("RulePanel").exists():
            for item in range(len(freeze_poco("RulePanel").child()) - 8):
                ...
                if freeze_poco("Title").exists() and \
                    ...
                    printcolor.printred("奖励规则界面缺少元素，请检查。。。")
                    screenshot.get_screen_shot(time.time(), devices, "奖励规则界面缺少元素")
            freeze_poco(texture="l_close_00").click()
        else:
            printcolor.printred("没有进入奖励界面，请检查。。。")
            screenshot.get_screen_shot(time.time(), devices, "没有进入奖励界面")
        if int(poco("GuildJokerDlg(Clone)").offspring("GameCount").child("Num").get_text()) != 0:
            ...
            printcolor.printgreen("小丑扑克没有游戏次数了")
    close(poco)


def Btnfish(poco, devices):  # 钓鱼
    freeze_poco = poco.freeze()  # TODO：定义dongjiepoco
    if poco("Title").exists():
        ...
        printcolor.printred("没有进入工会钓鱼界面，请检查。。。")
        screenshot.get_screen_shot(time.time(), devices, "没有进入工会钓鱼界面")


def BtnBuild(poco, devices):  # 建造
    freeze_poco = poco.freeze()  # TODO：定义dongjiepoco
            ...
            for item in range(len(freeze_poco("WrapContent").child()) - 8):
                ...
                if freeze_poco("GuildGrowthBuildDlg(Clone)").offspring(item1).exists():
                    ...
                    if poco("Title").exists():
                        ...
                        printcolor.printred("帮助界面没有打开，请检查。。。")
                        screenshot.get_screen_shot(time.time(), devices, "捐赠今日排行界面元素缺失")
...
def BtnConsider(poco, devices):  # 研究
    freeze_poco = poco.freeze()  # TODO：定义dongjiepoco
    if freeze_poco("Title").exists() and freeze_poco("Rukou0").exists() and freeze_poco("Rukou1").exists():
        ...
        if freeze_poco("GuildGrowthBuffDlg(Clone)").offspring("Detail").child("Name").exists() and \
            ...
            if freeze_poco("T").exists() and \
                ...
                printcolor.printred("工会研究所界面缺少元素，请检查。。。")
                screenshot.get_screen_shot(time.time(), devices, "工会研究所界面缺少元素")
                poco("Close").click()
        else:
            printcolor.printred("工会政厅界面缺少元素，请检查。。。")
            screenshot.get_screen_shot(time.time(), devices, "工会政厅界面缺少元素")
            poco("Close").click()
    else:
        printcolor.printred("没有弹出研究界面，请检查。。。")
        screenshot.get_screen_shot(time.time(), devices, "没有弹出研究界面")
