     ...
     if poco("SysEPVP").exists():
         ...
         with poco.freeze() as freezepoco:
             if freezepoco("Btn_MobaFamous").exists() and \
                 ...
                 printcolor.printgreen("竞技军衔，竞技名人堂，竞技副本显示正常")
             else:
                 printcolor.printred("竞技界面缺少元素，请详细查看")
             ...
             for i in name:
                 ...
                 if freezepoco(uiname).offspring("Desc").get_text() == "实时对战，领取称号和时装":
                     printcolor.printgreen("天梯赛--" + freezepoco(uiname).offspring("Desc").get_text())
                 elif freezepoco(uiname).offspring("Desc").get_text() == "巅峰对决，跨服对战":
                     printcolor.printgreen("角斗场--" + freezepoco(uiname).offspring("Desc").get_text())
                 elif freezepoco(uiname).offspring("Desc").get_text() == "4V4争夺据点，赢取龙币":
                     printcolor.printgreen("英雄战场--" + freezepoco(uiname).offspring("Desc").get_text())
                 elif freezepoco(uiname).offspring("Desc").get_text() == "释放操作，与全球玩家比拼":
                     printcolor.printgreen("全球竞技--" + freezepoco(uiname).offspring("Desc").get_text())
                 elif freezepoco(uiname).offspring("Desc").get_text() == "保护队长，获得传说装备":
                     printcolor.printgreen("保护队长--" + freezepoco(uiname).offspring("Desc").get_text())
                 else:
                     printcolor.printgreen("其他的PVP模式--" + freezepoco(uiname).offspring("Desc").get_text())
                 count += 1
         if poco("Btn_MobaRank").exists():
             ...
             with poco.freeze() as freezepoco:
                 if freezepoco("Date").exists() and \
                     ...
                     printcolor.printgreen("规则 赛季时间，军衔奖励，挑战记录，排名，军衔，角色，军功元素显示正常")
                 else:
                     printcolor.printred("规则 赛季时间，军衔奖励，挑战记录，排名，军衔，角色，军功元素缺少，请具体核对")
                     # screenshot.get_screen_shot(time.time(), devices, "竞技军衔按钮不存在")
             poco("RewardBtn").click()
             poco("ToggleResult").click()
             with poco.freeze() as freezepoco:
                 uiname = freezepoco("MilitaryRankDlg(Clone)").offspring("Panel").child()
                 print("打印达成的条件")
                 printcolor.printgreen("军衔           所需军功")
                 for i in uiname:
                     name = i.get_name()
                     if name[:4] == "item":
             ...
             with poco.freeze() as freezepoco:
                 ...
                 if count == 8:
                     printcolor.printgreen("赛季奖励的条目正常")
                 else:
                     printcolor.printred("赛季奖励的条目不是8条，请前往查看少了哪个")
                     # screenshot.get_screen_shot(time.time(), devices, "竞技军衔按钮不存在")
             touch((140,389))
             poco("Close").click()
         else:
             printcolor.printred("竞技军衔按钮不存在")
             # screenshot.get_screen_shot(time.time(), devices, "竞技军衔按钮不存在")
         if poco("Btn_MobaFamous").exists():
             ...
             with poco.freeze() as freezepoco:
                 if freezepoco("item0").exists() and \
                     ...
                     printcolor.printgreen("1:1天梯赛，英雄战场，全球竞技，跨服巅峰对决，点赞，规则，排行榜按钮显示正常")
                 else:
                     printcolor.printred("1:1天梯赛，英雄战场，全球竞技，跨服巅峰对决，点赞，规则，排行榜按钮显示异常")
                     # screenshot.get_screen_shot(time.time(), devices, "天梯赛报错异常")
             if poco("HallFameDlg(Clone)").offspring("Help").exists():
                    ...
                    with poco.freeze() as freezepoco:
                        ...
                        if freezepoco("Support").child("RedPoint").exists():
                            ...
                            if poco("ItemIconListBattleDlg(Clone)").exists():
                                printcolor.printgreen("点赞成功，拿到了点赞奖励")
                        ...
                        if freezepoco("RankList").exists():
                            ...
                            if poco(text="排名"):
                                printcolor.printgreen(f"排行榜打开正常")
                ...
                with poco.freeze() as freezepoco:
                    if freezepoco("SelectedTextLabel").exists() and \
                        ...
                        printcolor.printgreen("1:1天梯赛，评分奖励，排名奖励，荣誉商店，排行榜，对战记录，积分排名，开始匹配，技能教学，技能推荐按钮显示正常")
                    else:
                        printcolor.printred("1:1天梯赛，评分奖励，排名奖励，荣誉商店，排行榜，对战记录，积分排名，开始匹配，技能教学，技能推荐按钮显示异常")
                        # screenshot.get_screen_shot(time.time(), devices, "天梯赛报错")
                        try:
                            ...
                            poco("Match1V1Btn").click()
                            poco("Close").click()
                            printcolor.printgreen("天梯赛所有按钮点击关闭操作正常")
                ...
                with poco.freeze() as freezepoco:
                    if freezepoco("Help").exists() and \
                            ...
                            freezepoco("AbattoirDlg(Clone)").offspring("Reward").child("item0").exists() and \
                            freezepoco("AbattoirDlg(Clone)").offspring("Reward").child("item1").exists() and \
                            freezepoco("AbattoirDlg(Clone)").offspring("Reward").child("item2").exists() and \
                            freezepoco("AbattoirDlg(Clone)").offspring("WinOfPoint").child("Num"):
                        printcolor.printgreen("三个排名奖励，升段奖励，排行榜，荣誉商店，参与宝箱，胜利宝箱，技能推荐按钮显示正常")
                    else:
                        printcolor.printred("三个排名奖励，升段奖励，排行榜，荣誉商店，参与宝箱，胜利宝箱，技能推荐按钮显示异常")
                        # screenshot.get_screen_shot(time.time(), devices, "角斗场报错")
                        try:
                            poco("Help").click()
                            poco("Btn").click()
                            ...
                            if poco("Pause").exists():
                                ...
                                time.sleep(15)
                                poco("SysEPVP").click()
                                printcolor.printgreen("角斗场进入退出正常")
                            else:
                ...
                with poco.freeze() as freezepoco:
                    if freezepoco("Help").exists() and \
                        ...
                        printcolor.printgreen("规则，玩法攻略，技能预览，排行榜，商店，挑战记录，奖励预览，本周奖励，每日首胜，单人匹配，组队匹配，购买按钮，英雄列表等元素显示正常")
                    else:
                        printcolor.printred("规则，玩法攻略，技能预览，排行榜，商店，挑战记录，奖励预览，本周奖励，每日首胜，单人匹配，组队匹配，购买按钮，英雄列表等元素显示异常")
                        # screenshot.get_screen_shot(time.time(), devices, "英雄战场点击报错")
                        try:
                            poco("Help").click()
                            poco("Btn").click()
                            poco("RankBtn").click()
                            poco(texture="l_close_00").click()
                            poco("HeroBattleDlg(Clone)").offspring("ShopBtn").child("P")[1].click()
                            poco("Close").click()
                            poco("RecordBtn")
                            poco(texture="l_close_00").click()
                            ...
                            poco("Close").click()
                            printcolor.printgreen("英雄战场所有按钮点击关闭操作正常")
                        except Exception as e:
                            printcolor.printred("英雄战场界面元素点击报错")
                            printcolor.printred(e)
                            # screenshot.get_screen_shot(time.time(), devices, "英雄战场界面元素点击报错")
                ...
                with poco.freeze() as freezepoco:
                    if freezepoco("0").exists() and \
                            ...
                            freezepoco("t").exists():
                        printcolor.printgreen("大标题，介绍内容，全球榜，排行榜，前往挑战按钮，可能获得的奖励显示正常")
                    else:
                        printcolor.printred("大标题，介绍内容，全球榜，排行榜，前往挑战按钮，可能获得的奖励等元素显示异常")
                        # screenshot.get_screen_shot(time.time(), devices, "全球竞技缺少对应按钮")
                        try:
                            ...
                            poco("Close").click()
                            poco("Close").click()
                            printcolor.printgreen("全球竞技缺所有按钮点击关闭操作正常")
                        except Exception as e:
                            printcolor.printred("全球竞技缺界面元素点击报错")
                            printcolor.printred(e)
                ...
                with poco.freeze() as freezepoco:
                    if freezepoco("CaptainDlg(Clone)").offspring("WeekReward").child("ListPanel").exists() and \
                        ...
                        printcolor.printgreen("周奖励，日奖励，单人匹配，组队匹配，荣誉商店，对战记录，规则，场次计算显示正常")
                    else:
                        printcolor.printred("周奖励，日奖励，单人匹配，组队匹配，荣誉商店，对战记录，规则，场次计算显示正常")
                        # screenshot.get_screen_shot(time.time(), devices, "保护队长元素报错")
                        try:
                            poco("Help").click()
                            poco("Btn").click()
                            poco("BtnShop").click()
                            ...
                            poco("Close").click()
                            printcolor.printgreen("保护队长所有按钮点击关闭操作正常")
                        except Exception as e:
                            printcolor.printred("保护队长界面元素点击报错")
                            printcolor.printred(e)
                            # screenshot.get_screen_shot(time.time(), devices, "保护队长报错")
             else:
                printcolor.printred("界面缺少元素，麻烦检查")
                # screenshot.get_screen_shot(time.time(), devices, "竞技军衔按钮不存在")
         else:
             printcolor.printred("竞技名人堂按钮不存在")
             # screenshot.get_screen_shot(time.time(), devices, "竞技名人堂按钮不存在")
     else:
