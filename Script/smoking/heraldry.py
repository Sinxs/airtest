...
def addheraldry(poco,devices):
    ...
    if but.exists():
        ...
        if poco("EquipCreateDlg(Clone)").offspring("Item").child("Icon").exists():  # 制作窗口是否存在金属板
            printcolor.printgreen("开始进行纹章制作")
            ...
            if poco("EquipCreateDlg(Clone)").offspring("EquipSetCreateConfirmFrame").child("Bg").exists():  # 是否存在制作纹章的弹窗
                ...
                printcolor.printred("没有弹出金属板制作弹窗，请检查。。")
                screenshot.get_screen_shot(time.time(), devices, "没有弹出金属板制作弹窗")
    else:
        if not but.exists():  # 如果界面不存在金属板
            ...
            if poco("ListPanel").exists():  # 存在获取途径
                ...
                if poco(text="当前挑战BOSS").exists():
                    printcolor.printgreen("进入bossrush界面，进行bossrush判断")
                    if int(poco("BossRushNewDlg(Clone)").child("Num").get_text()[0]) != 0:
                        printcolor.printgreen("进入bossrush副本")
                        poco("Go").click()  # 点击挑战
            ...
            if poco("ItemNewDlg(Clone)").offspring("Items").child("Frame").offspring("T").exists():
                ...
                if poco("ItemNewDlg(Clone)").offspring("Items").offspring("Icon").exists():
                    ...
                    if poco("main").exists():  # 打开纹章装备界面
                        ...
                        # screenshot.get_screen_shot(time.time(), devices, "装备纹章截图验证")
                        ...
                        printcolor.printgreen("点击纹章后没有打开纹章装备界面")
                        screenshot.get_screen_shot(time.time(), devices, "点击纹章后没有打开纹章装备界面")
                else:
                    printcolor.printred("没有纹章")
                    screenshot.get_screen_shot(time.time(), devices, "没有纹章")
        else:
