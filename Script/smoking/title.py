    ...
    if poco("Title").exists():
        ...
        if poco("XSys_Design_Designation").exists():
            ...
            if poco(text="日常称号").exists():
                printcolor.printgreen("进入称号界面，开始检测称号。。。")
                for i in range(1, 7):
                        ...
                        for ii in range(len(freeze_poco("WrapContent").child())):
                            ...
                            if title.exists():  # 判断称号存在
                                ...
                                if "fff" in title:
                                    ...
                                    print("图片称号显示错误")
                                    screenshot.get_screen_shot(time.time(), devices, "没有进入角色界面")
                    poco("ItemNewDlg(Clone)").offspring(str(i)).child("Bg").click()
                printcolor.printgreen("称号检测完毕。。。没有发现异常")
    else:
        printcolor.printred("没有进入角色界面，请检查")
        screenshot.get_screen_shot(time.time(), devices, "没有进入角色界面")
