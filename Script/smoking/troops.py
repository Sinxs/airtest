...
def troops(devices):
    ...
    if poco("Team").exists():  # 队伍按钮
        ...
        if poco("BtnLeave").exists():
            printcolor.printgreen("已经有了队伍，先把队伍解散")
            poco("BtnLeave").click()  # 如果已经创建了队伍，就点击离队取消队伍
    else:
        printcolor.printgreen("主界面没有队伍按钮，请检查")
        screenshot.get_screen_shot(time.time(), devices, "主界面没有队伍按钮")
    ...
    if poco("Team").exists():  # 队伍按钮
        ...
        if freeze_poco("RecruitDlg(Clone)").child("Bg").offspring("Text")[1].exists() and \
            ...
            printcolor.printgreen("进入招募大厅大厅成功\n开始检测下级界面《队友验证》")
                ...
                if poco("RecruitDlg(Clone)").child("Bg").offspring("Text")[1].exists():
                    ...
                    if freeze_poco(text="队员招募").exists() and freeze_poco(text="队员招募").exists() and \
                        ...
                        for Label in freeze_poco("RecruitGroupPublishFrame(Clone)").offspring("1").offspring(
                            ...
                            printcolor.printgreen("检查点 《"+Label.get_text()+"》 显示正确")
                        ...
                        for Label in freeze_poco("RecruitGroupPublishFrame(Clone)").offspring("3").offspring(
                                "ChildList").offspring("Selected").offspring("Label"):
                            printcolor.printgreen("检查点 《"+Label.get_text()+"》 显示正确")
                        printcolor.printgreen("检查点 《发布招募》 界面显示正确\n点击返回到上级界面")
                        ...
                        if poco("ToggleMember").exists():  # 寻找队友
                            ...
                            if freeze_poco("Title0").exists() and \
                                ...
                                printcolor.printgreen("检查点 《寻找队友》 界面显示正确")
                                poco("ToggleTeam").click()  # 点击寻找队伍，回到上级界面，为下次测试恢复环境
                            else:
                                printcolor.printred("寻找队友界面缺少按钮控件元素，请检查")
                                screenshot.get_screen_shot(time.time(), devices, "队友验证界面缺少按钮控件元素")

            else:
                printcolor.printred("队友验证界面缺少按钮控件元素，请检查")
                screenshot.get_screen_shot(time.time(), devices, "队友验证界面缺少按钮控件元素")
        else:
            printcolor.printred("招募大厅界面缺少按钮控件元素，请检查")
            screenshot.get_screen_shot(time.time(), devices, "招募大厅界面缺少按钮控件元素")
    else:
        printcolor.printred("主界面没有队伍按钮，请检查")
        screenshot.get_screen_shot(time.time(), devices, "主界面没有队伍按钮")
